...
describe("Firebase Rules", function () {
  ...
  describe("/docs", async () => {
    ...
    describe("/docs/<docID>", async () => {
      ...
      describe("/docs/<docID>/images", async () => {
        it("Can read all images if user has access", async () => {
          await assertSucceeds(
            Promise.all([
              get(ref(myContext.database(), `docs/${myDoc}/images`)),
              get(ref(viewerContext.database(), `docs/${myDoc}/images`)),
              get(ref(editorContext.database(), `docs/${myDoc}/images`)),
            ])
          );
        });
        it("Can't read all images if user doesn't have access", async () => {
          await assertFails(
            allRejected([
              get(ref(theirContext.database(), `docs/${myDoc}/images`)),
              get(ref(unauthContext.database(), `docs/${myDoc}/images`)),
            ])
          );
        });
        it("Can't arbitrarily set all images", async () => {
          await assertFails(
            set(ref(theirContext.database(), `docs/${theirDoc}/images`), {
              badAppleLink: false,
              badLink: true,
            })
          );
